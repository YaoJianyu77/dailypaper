...
Ignored `.cache/dailypaper/runs/<date>/` holds `runtime.json`, selection, PDFs/page images, analyzed-paper checkpoints, trends, the rendered site, and `bundle.json`. Runtime receipts freeze model, reasoning, CLI version, and settings digest. Fresh papers receive one analysis call; a schema-valid analysis that fails deterministic validation is retained, stops the run, and can receive one targeted correction on an explicit resume. A schema-invalid result stops without an automatic model retry. Use `run_local_daily.py --date YYYY-MM-DD` to resume an unfinished report after the local date changes. Report, screenshot, and asset hashes bind archived files to validated artifacts; transaction recovery follows AGENTS.md.
