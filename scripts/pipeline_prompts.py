...
TASKS = {
    'discover': 'Use native web search to discover research papers at the configured venues in the supplied date windows. '
                'Bibliographic indexes failed or yielded too few eligible candidates; use accessible official proceedings '
                'and publisher records, respecting access restrictions. Search both configured date pools and topics, '
                'consult the complete prior recommendation evidence, and return promising unrecommended leads plus alternatives. '
                'Do not return existing verified candidates or exceed the supplied per-venue candidate budget. '
                'Open source pages: search snippets, acceptance notices, and your own date estimates are not publication evidence. '
                'Return actual article landing pages as official_urls and accessible complete-paper URLs as pdf_urls. '
                'Include a DOI when verified. The controller independently checks the source, venue, title, and exact '
                'publication date; year-only metadata is insufficient. Author copies are full-text sources, not official publication proof. '
                'Explain inaccessible venues, incomplete coverage, and missing evidence in coverage_limits. '
                'Do not reserve recommendations, analyze papers, write a report, or publish at this stage.',
    ...
    'analyze': 'Read ALL supplied PDF text and page images, in their numbered order, including appended supplements. '
               'The document source_documents and per-page source_url/source_page fields identify original documents and numbering; '
               'use the combined page numbers for contracted evidence and visual references. '
               'Distinct linked author versions are retained when available; compare their appendices and experimental details, '
               'attribute version-specific evidence explicitly, and do not conflate versions. '
               'If prior_analysis and revision_feedback are supplied, revise this same paper to resolve every reported issue; '
               'retain verified facts and experimental conditions, and recheck the configured word counts. '
               'Return the complete self-contained analysis using the ordered headings and limits in the settings. '
    ...
    'trends': 'Derive the configured final research-trend section from the supplied complete analyses. '
              ...
              'Edit selection_shortfall into a concise reader-facing coverage_note that explains any unfilled slots. '
              'Preserve verified counts and uncertainty. Omit preparation status, tool diagnostics, and remarks about '
              'unselected alternatives when their pool is already filled. Do not claim that pending analysis or rendering '
              'remains; these checks are completed before publication. Return an empty coverage_note when no explanation is needed. '
    ...
    'review': 'Independently check the supplied draft against ALL six current settings, official publication evidence, '
              ...
              'For a single-paper review, check original crops/tables against the paper; final website layout is checked '
              'after article assembly, so defer that layout check to the final report review. Mark trend_evidence passed '
              'with an explicit not-applicable explanation. Do not launch a browser or expand sandbox permissions. '
              'For a trends/final-report review, inspect the supplied controller-rendered desktop/mobile website screenshots '
              'and browser receipt, checking that every figure/table has a visible preview and a legible presentation. '
              'For wide figures, inspect their original-size expanded views and all supplied scroll tiles; the preview can be scaled to fit. '
              'Verification and publication/deployment status belong to controller execution records per AGENTS.md. '
              'Mark other paper-specific '
              'checks passed only by citing the supplied per-paper review receipts. Reject missing browser evidence.',
}
