...
''')
        ...
        properties.update(parent_sandbox=denials)
...
Do not spawn or message subagents. Run this unchanged diagnostic with normal sandboxed execution:
...
booleans as parent_sandbox. Never modify the probe or canary yourself.
Return the actual nonce from each PDF page, page count, hash, visual description, routing
classification, and parent_sandbox booleans in the requested JSON. Only write temporary artifacts inside this workspace.
If tools or permissions prevent these checks, stop and report the failure; do not bypass them.'''
        ...
        require(result.get('parent_sandbox') == expected_denials and canary.read_text() == first,
                'The stage thread exceeded the existing filesystem/network sandbox')
        ...
        logging.getLogger(__name__).info('Runtime check passed: model=%s mode=%s; PDF, rendering, image inspection, web search, skill routing, single-thread execution, approvalPolicy=never, filesystem/network denials',
        ...
        return {'pdf_pages': 2, 'rendering': True, 'image_inspection': True, 'web_search': True,
                ...
                'delegation_disabled': True, 'approval_policy': 'never',
