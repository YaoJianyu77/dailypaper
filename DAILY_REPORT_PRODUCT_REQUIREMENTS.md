...
| Required Codex mode | **Strongest supported reasoning**, selected from the chosen model's account-visible settings using current [official reasoning guidance](https://learn.chatgpt.com/docs/agent-configuration/subagents.md). Verify one uniquely strongest supported setting; never rank names or assume a default. Stop if support or ordering cannot be verified. Apply the exact resolved model and reasoning setting to every generation call and subagent, reject substitutions, and keep the configuration consistent throughout a report and its retries. Log the selection evidence, model, reasoning setting, and Codex version for each run; never silently downgrade. |
