...
The dry-run prepares and checks an article in temporary storage. The normal run verifies the current recommended model with balanced `medium` reasoning, keeps every stage in one agent thread, generates the report, commits, and pushes. It uses one complete-paper analysis per selected paper and does not spend model calls on independent reviews or the synthetic installation diagnostic. Run `--check-runtime` explicitly after runtime changes. `--date` resumes a specific unfinished local date after midnight. Runtime mismatches stop the run without a downgrade. GitHub Pages deploys the committed content.
