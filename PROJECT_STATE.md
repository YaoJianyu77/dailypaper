...
- The suite passes 86 isolated tests, covering complete Codex inputs, settings/skill edits, review routing, retry recovery, permanent exclusions, permissions, scheduling, and browser rendering. A focused end-to-end test also verifies that final review receives the complete article once, with paper-review receipts and browser evidence intact. Cached analyses that fail current validation are revised and reviewed again, and an explicit local date can resume an unfinished report after midnight. Five canonical skills validate and remain discoverable through links; their retained image helper passes an isolated PDF extraction check.
