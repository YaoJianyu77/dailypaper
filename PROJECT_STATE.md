...
- All 52 existing reports still build; their 51 image references are retained. Production content/state and the GitHub workflows remain unchanged by this upgrade. The earlier cleanup verified all 1,127 tracked content/state, cron-helper and Pages-workflow files against its baseline (`539994c`); this upgrade intentionally replaces the local cron helper.
...
- The existing manual GitHub Models Action cannot verify Codex Ultra and is rejected by the production runner. Its schedule and credentials were retained; use the authenticated local job for production. The Pages workflow continues to build archived content independently.
